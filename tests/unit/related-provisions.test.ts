...
describe("related provisions", () => {
  it("surfaces rules provisions directly and indirectly related to Act section 86", async () => {
    ...
    const related = index["aged-care-act-2024:seg:section-86-priority-category-decisions"] ?? [];
  ...
  it("surfaces the Act provision cited by Rules 87-5", async () => {
    const index = await getRelatedProvisionIndex();
    const related =
      index["aged-care-rules-2025:seg:rule-87-5-priority-categories-and-eligibility-criteria-for-classification-type-ongoing"] ??
      [];
